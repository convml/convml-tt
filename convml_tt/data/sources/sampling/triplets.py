...
def generate_tile_domain_with_peturbed_location(domain, tile_domain, tile_size, distance_size_scaling):
    """
    Generate a tile of a specific `tile_siez` that fits inside `domain`
    """
    domain_bounds_geometry = domain.spatial_bounds_geometry
    ...
    if isinstance(domain, LocalCartesianDomain):
        tile_domain_perturbed = tile_domain_perturbed.locate_in_latlon_domain(domain=domain)

    if domain_bounds_geometry.contains(tile_domain_perturbed.spatial_bounds_geometry):
        return tile_domain_perturbed
    else:
        return generate_tile_domain_with_peturbed_location(domain=domain, tile_domain=tile_domain, tile_size=tile_size, distance_size_scaling=distance_size_scaling)
...
def generate_triplet_location(
    ...
    anchor_tile_domain = generate_randomly_located_tile(domain=domain, tile_size=tile_size)
    neighbor_tile_domain = generate_tile_domain_with_peturbed_location(domain=domain, tile_domain=anchor_tile_domain, tile_size=tile_size, distance_size_scaling=neigh_dist_scaling)
    distant_tile_domain = generate_randomly_located_tile(domain=domain, tile_size=tile_size)
