...
from fastcore.dispatch import Tuple, delegates, patch
from fastcore.transform import DisplayedTransform
from fastcore.utils import is_listy
from PIL import Image
from torch import Tensor
from torch import ones_like as t1
from torch import stack
from torch import zeros_like as t0
...
class RandTransform(DisplayedTransform):
    ...
    def __init__(self, p=1.0, nm=None, before_call=None, **kwargs):
        super().__init__(**kwargs)
...
class RandomCrop(RandTransform):
    ...
    def before_call(self, b, split_idx):
        ...
        if split_idx:
            ...
            self.tl = Tuple(
                random.randint(0, self.orig_sz[0] - self.size[0]),
                random.randint(0, self.orig_sz[1] - self.size[1]),
            )
...
class Resize(RandTransform):
    split_idx, mode, mode_mask, order, store_attrs = (
        None,
        Image.BILINEAR,
        Image.NEAREST,
        1,
        "size,method,pad_mode",
    )
...
class RandomResizedCrop(RandTransform):
    ...
    def before_call(self, b, split_idx):
        ...
        for attempt in range(10):
            area = random.uniform(self.min_scale, 1.0) * w * h
            ratio = math.exp(
...
# Cell
def mask_tensor(x, p=0.5, neutral=0.0, batch=False):
...
def zoom_mat(
    ...
    def _def_draw(x):
        return x.new(x.size(0)).uniform_(min_zoom, max_zoom)
...
class LightingTfm(RandTransform):
    ...
    def encodes(self, x: TensorImage):
        return x.lighting(partial(compose_tfms, tfms=self.fs))
...
class _BrightnessLogit:
    ...
    def _def_draw(self, x):
        if not self.batch:
            return x.new(x.size(0)).uniform_(
...
class RandomErasing(RandTransform):
    ...
    def __init__(self, p=0.5, sl=0.0, sh=0.3, min_aspect=0.3, max_count=1):
        super().__init__(p=p)
