...
def test_train_new():
    trainer = pl.Trainer(max_epochs=5, devices=N_GPUS)
    arch = "resnet18"
...
def test_finetune_pretrained():
    trainer = pl.Trainer(max_epochs=5, callbacks=[HeadFineTuner()], devices=N_GPUS)
    arch = "resnet18"
...
def test_train_new_onecycle():
    ...
    trainer = OneCycleTrainer(max_epochs=5, callbacks=[lr_monitor], devices=N_GPUS)
    arch = "resnet18"
    model = TripletTrainerModel(pretrained=False, base_arch=arch)
    data_path = fetch_example_dataset(dataset=ExampleData.TINY10)
    datamodule = TripletTrainerDataModule(
